...
export const UnlockEthForm = forwardRef(({ props: { Eth, senderAddress, loading, caETHContract, caETHabi } }, ref) => {
  const { wallet, signedAccountId } = useContext(NearContext);
  const [amount, setAmount] = useState(0);

  useImperativeHandle(ref, () => ({
    async createPayload() {
      const data = Eth.createTransactionData(caETHContract, caETHabi, 'unlockTokens', [amount]);
      console.log(data)
  ...
  return (
    <>
      <h5 className="text-center">Unlock ETH</h5>
...
UnlockEthForm.displayName = 'EthereumContractView';