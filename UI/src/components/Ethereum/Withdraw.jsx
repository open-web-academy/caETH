...
export const WithdrawForm = forwardRef(({ props: { Eth, senderAddress, loading, caETHContract, caETHabi } }, ref) => {
  const { wallet, signedAccountId } = useContext(NearContext);
  const [amount, setAmount] = useState(0);

  useImperativeHandle(ref, () => ({
    async createPayload() {
      const data = Eth.createTransactionData(caETHContract, caETHabi, 'withdraw', [amount]);
      console.log(data)
  ...
  return (
    <>
      <h5 className="text-center">Withdraw ETH</h5>
...
WithdrawForm.displayName = 'EthereumContractView';